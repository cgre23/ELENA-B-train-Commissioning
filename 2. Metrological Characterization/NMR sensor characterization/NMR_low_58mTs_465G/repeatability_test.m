...
trigger_times_op = zeros(n,1);
trigger_currents_op = zeros(n,1);
minimumpeak_op = zeros(n,1);
minimumpeak_sp = zeros(n,1);
trigger_diff_op = zeros(n,1);
...
current = DCCT_op;
rmsCurrent = snr(current);
current_nf = DCCT_op *100;

% Plot  voltage
% plot(time, nmr_sp, time, nmr_op);
% xlabel('Time (s)');
% ylabel(' Current (A)');
...
time_index_NMRop = find(nmr_op < 0,900);
time_index_NMRsp = find(nmr_sp < 0.07, 900);
