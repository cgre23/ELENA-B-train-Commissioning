...
filename = sprintf('2000G/NMRanalysis1_2000G_%d.mat',k);
...
current = variable(:, 2);
 ...
 plot(time, nmr-0.2324, '-r', time, fmr+0.2473, '-k', time,trigger_nmr,'-b', time, trigger_fmr, '-g');
xlabel('Time (s)', 'interpreter','latex');
ylabel('Voltage (V)', 'interpreter','latex');
legend('NMR signals', 'FMR signals','NMR trigger signals', 'FMR trigger signals');
xlim([2.711 2.7140]);
hold on;
