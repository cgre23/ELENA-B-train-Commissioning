...
%% Load data
r3=2700:3816;  r4=2600:4200;  r6=3100:4700;  r9=3400:5400;  r12=3800:6000;  r15=4300:7000;  r18=4500:6900; 
load('data/Cycle_40A_100As.mat');
I40 = curr(r3);
B40 = BdL(r3);
t40 = tim(r3); grad_est = grad_est';
g40 = grad_est(r3);
load('data/Cycle_50A_100As.mat'); 
I50 = curr(r4);
B50 = BdL(r4);
t50 = tim(r4); grad_est = grad_est';
g50 = grad_est(r4);
load('data/Cycle_100A_100As.mat'); 
I100 = curr(r6);
B100 = BdL(r6);
t100 = tim(r6); grad_est = grad_est';
g100 = grad_est(r6);
load('data/Cycle_150A_100As.mat'); 
I150 = curr(r9);
B150 = BdL(r9);
t150 = tim(r9); grad_est = grad_est';
g150 = grad_est(r9);
load('data/Cycle_200A_100As.mat');
I200 = curr(r12);
B200 = BdL(r12);
t200 = tim(r12); grad_est = grad_est';
g200 = grad_est(r12);
load('data/Cycle_250A_100As.mat'); 
I250 = curr(r15);
B250 = BdL(r15);
t250 = tim(r15); grad_est = grad_est';
g250 = grad_est(r15);
load('data/Cycle_276A_100As.mat'); 
I276 = curr(r18);
B276 = BdL(r18);
...
[v i276d] = max(diff(g276(501:end)));
[v i250d] = max(diff(g250(501:end)));
[v i200d] = max(diff(g200(501:end)));
[v i150d] = max(diff(g150(501:end)));
[v i100d] = max(diff(g100(501:end)));
[v i50d] = max(diff(g50(501:end)));
[v i40d] = max(diff(g40(501:end)));
   
i276d = i276d+500; i250d = i250d+500; i200d = i200d+500; i150d = i150d+500; i100d = i100d+500; i50d = i50d+500; i40d = i40d+500;
...
% 
et = linspace(0, length(e40up)/200, length(e40up));
etd = linspace(0, length(e40d)/200, length(e40d));
...
exp_mod_up = t1(v)*(1-(t2(v)*exp(-t3(v)*et))-(t4(v)*exp(-t5(v)*et)));
exp_mod_down = t6(v)*(1-(t7(v)*exp(-t8(v)*et))-(t9(v)*exp(-t10(v)*et)));
 plot(et, exp_mod_up, 'b--', 'LineWidth',2); hold on; plot(et, e200n, 'r');  grid on; hold on;
plot(etd, exp_mod_down, 'b--', 'LineWidth',2); hold on; plot(etd, e200dn, 'r');   hold on;

v = 250; 
exp_mod_up = t1(v)*(1-(t2(v)*exp(-t3(v)*et))-(t4(v)*exp(-t5(v)*et)));
exp_mod_down = t6(v)*(1-(t7(v)*exp(-t8(v)*et))-(t9(v)*exp(-t10(v)*et)));
 plot(et, exp_mod_up, 'b--', 'LineWidth',2); hold on; plot(et, e250n, 'r');  grid on; hold on;
plot(etd, exp_mod_down, 'b--', 'LineWidth',2); hold on; plot(etd, e250dn, 'r');   hold on;

v = 276; 
exp_mod_up = t1(v)*(1-(t2(v)*exp(-t3(v)*et))-(t4(v)*exp(-t5(v)*et)));
exp_mod_down = t6(v)*(1-(t7(v)*exp(-t8(v)*et))-(t9(v)*exp(-t10(v)*et)));
 plot(et, exp_mod_up, 'b--', 'LineWidth',2); hold on; plot(et, e276n, 'r');  grid on; hold on;
plot(etd, exp_mod_down, 'b--', 'LineWidth',2); hold on; plot(etd, e276dn, 'r');   hold on;
